function [center, U, obj_fcn] = udfcm_s1(data, cluster_n,a,options)
...
U = udinitfcm_s1(cluster_n, data_n);			% Initial fuzzy partition
% Main loop
for i = 1:max_iter,
	[U, center, obj_fcn(i)] = udstepfcm_s1(data,data_mean, U, cluster_n,a,expo);
	if display, 
...
function out=dm(x,y,d)
 out=(d(x-1,y)+d(x-1,y-1)+d(x-1,y+1)+d(x+1,y)+d(x+1,y-1)+d(x+1,y+1)+d(x,y-1)+d(x,y+1))/8;
