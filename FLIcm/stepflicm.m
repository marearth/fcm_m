function [U_new, center, obj_fcn] = stepflicm(data,rc, U, cluster_n, expo)
...
[dist1,dist2] = uddistfcm(center, data,rc,U,expo);       % fill the distance matrix
obj_fcn = sum(sum((dist1.^2).*mf))+sum(sum(dist2));  % objective function
dist=dist1.^2+dist2;
